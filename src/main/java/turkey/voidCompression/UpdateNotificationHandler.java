...
public class UpdateNotificationHandler
{
	...
	public void onPlayerLogin(PlayerLoggedInEvent event)
	{
		...
		if(MPACore.VERSION == "SM_VER" + "_KEY")
		{
			event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "[SimplyMagic] Dev Copy, How did you get this?"));
			return;
		}
		...
		try
		{
			...
			if (verStat == -1)
			{
				...
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "[SimplyMagic] Version " + version + " of SimplyMagic is now available!"));
			...
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN + "[SimplyMagic] " + version + " is up to date"));
			}
			else if (verStat == 1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "[SimplyMagic] " + version + " is a dev build"));
			}
			else if (verStat == -2)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "[SimplyMagic] An error has occured while checking the version for SimplyMagic!"));
			}

		} catch (Exception e)
		{
			event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.DARK_RED + "[SimplyMagic] A critical error has occured while checking the version for SimplyMagic!"));
			e.printStackTrace();
			return;
		}*/
	}
